...
JARVIS_DOCKER_BUILD_USE_BUILDX="${JARVIS_DOCKER_BUILD_USE_BUILDX:-0}"
JARVIS_DOCKER_BUILD_CACHE_FROM="${JARVIS_DOCKER_BUILD_CACHE_FROM:-}"
JARVIS_DOCKER_BUILD_CACHE_TO="${JARVIS_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${JARVIS_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${JARVIS_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${JARVIS_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${JARVIS_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${JARVIS_DOCKER_BUILD_CACHE_TO}")
  fi
fi
