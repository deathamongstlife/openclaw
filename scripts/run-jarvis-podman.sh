...
# NOTE: must be evaluated after sourcing ENV_FILE so JARVIS_GATEWAY_BIND set in .env takes effect.
...
SELINUX_MOUNT_OPTS=""
if [[ -z "${JARVIS_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  # Honour explicit override (e.g. JARVIS_BIND_MOUNT_OPTIONS=":Z" → strip leading colon for inline use).
  SELINUX_MOUNT_OPTS="${JARVIS_BIND_MOUNT_OPTIONS#:}"
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -v "$CONFIG_DIR:/home/node/.jarvis:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.jarvis/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "${ENV_FILE_ARGS[@]}" \
    "$JARVIS_IMAGE" \
