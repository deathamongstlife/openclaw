...
const EXPECTED_REPOSITORY_URL = "https://github.com/jarvis/jarvis";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "jarvis") {
    errors.push(`package.json name must be "jarvis"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.jarvis !== "jarvis.mjs") {
    errors.push(`package.json bin.jarvis must be "jarvis.mjs"; found "${pkg.bin?.jarvis ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`jarvis-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `jarvis-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
