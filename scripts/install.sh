...
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.ai/install.sh | bash
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://jarvis.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.jarvis.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.ai/install.sh | bash -s -- [options]
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://jarvis.ai/install.sh | bash"
}
...
if ! declare -F extract_jarvis_semver >/dev/null 2>&1; then
...
resolve_jarvis_version() {
    ...
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_jarvis_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
...
main() {
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.ai/install.sh | bash -s -- --install-method npm"
