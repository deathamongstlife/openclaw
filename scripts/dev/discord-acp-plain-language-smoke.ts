...
type DriverMode = "token" | "webhook" | "jarvis";
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver jarvis] [options]\n\n" +
    ...
    "  --driver <token|webhook|jarvis> Driver transport mode (default: token)\n" +
    ...
    "  --jarvis-bin <path>        Jarvis CLI binary for driver=jarvis (default: jarvis)\n" +
    ...
    "  JARVIS_DISCORD_SMOKE_JARVIS_BIN"
  );
}
...
function parseArgs(): Args {
  ...
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.JARVIS_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent =
    resolveArg("--agent") ||
  ...
  const threadBindingsPath =
    ...
    defaultBindingsPath;
  const jarvisBin =
    resolveArg("--jarvis-bin") || process.env.JARVIS_DISCORD_SMOKE_JARVIS_BIN || "jarvis";
...
async function jarvisCliJson<T>(params: { jarvisBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.jarvisBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`jarvis ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await jarvisCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    jarvisBin: params.jarvisBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "jarvis") {
    return await readMessagesWithOpenclaw({
      jarvisBin: params.args.jarvisBin,
...
async function run(): Promise<SuccessResult | FailureResult> {
  ...
  try {
    if (args.driverMode === "token") {
      ...
      setupStage = "send-message";
      const sent = await jarvisCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("jarvis message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
