...
describe("control UI routing", () => {
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("jarvis.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/jarvis" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("jarvis.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/jarvis",
    });
    expect(JSON.parse(localStorage.getItem("jarvis.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/jarvis";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/jarvis");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp("/ui/overview?gatewayUrl=wss://other-gateway.example/jarvis#token=abc123");
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/jarvis");
    expect(app.settings.token).toBe("");
