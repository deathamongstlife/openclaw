...
final class PeekabooBridgeHostCoordinator {
    ...
    private static var jarvisSocketPath: String {
        ...
        return Self.makeSocketPath(for: "Jarvis", in: base)
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.jarvisSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.jarvisSocketPath)
        } catch {
