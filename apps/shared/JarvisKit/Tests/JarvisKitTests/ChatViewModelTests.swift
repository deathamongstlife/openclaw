...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> JarvisChatHistoryPayload
{
    JarvisChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> JarvisChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> JarvisChatSessionEntry
{
    JarvisChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> JarvisChatModelChoice {
    JarvisChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}
...
private func sendUserMessage(_ vm: JarvisChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, JarvisChatTransport {
    ...
    private let modelResponses: [[JarvisChatModelChoice]]
    ...
    func listModels() async throws -> [JarvisChatModelChoice] {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = JarvisChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: JarvisChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(JarvisChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == JarvisChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = JarvisChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: JarvisChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = JarvisChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = JarvisChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
