...
public final class JarvisChatViewModel {
    ...
    public private(set) var modelChoices: [JarvisChatModelChoice] = []
    ...
    private var sessionDefaults: JarvisChatSessionsDefaults?
    ...
    public init(
        sessionKey: String,
        transport: any JarvisChatTransport,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = JarvisChatSessionEntry(
                key: current.key,
                kind: current.kind,
