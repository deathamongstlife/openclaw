...
struct JarvisLiveActivity: Widget {
    ...
    @ViewBuilder
    private func trailingView(state: JarvisActivityAttributes.ContentState) -> some View {
    ...
    @ViewBuilder
    private func statusDot(state: JarvisActivityAttributes.ContentState) -> some View {
        Circle()
            .fill(dotColor(state: state))
            .frame(width: 6, height: 6)
    }

    private func dotColor(state: JarvisActivityAttributes.ContentState) -> Color {
