...
private const val NOTIFICATION_CHANNEL_BASE_ID = "jarvis.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Jarvis Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Jarvis Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Jarvis Active")
      }
