...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      // System command
      JarvisSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.jarvis.app.protocol.JarvisPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      JarvisContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      JarvisContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      JarvisCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      JarvisCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      JarvisMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      JarvisMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)
