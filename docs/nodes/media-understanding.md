...
When provider-based **audio** and **video** media understanding is enabled, Jarvis
...
If the proxy value is malformed, Jarvis logs a warning and falls back to direct
fetch.
...
- `parakeet-mlx` note: with `--output-dir`, Jarvis reads `<output-dir>/<media-basename>.txt` when output format is `txt` (or unspecified); non-`txt` formats fall back to stdout.
