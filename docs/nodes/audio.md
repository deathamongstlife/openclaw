...
If no proxy env vars are set, direct egress is used. If proxy config is malformed, Jarvis logs a warning and falls back to direct fetch.
...
- For `parakeet-mlx`, if you pass `--output-dir`, Jarvis reads `<output-dir>/<media-basename>.txt` when `--output-format` is `txt` (or omitted); non-`txt` output formats fall back to stdout parsing.
