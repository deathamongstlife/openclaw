...
- For direct shell/runtime file executions, Jarvis also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If Jarvis cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
jarvis nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
jarvis nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
jarvis nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
