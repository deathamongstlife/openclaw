...
jarvis onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`JARVIS_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
export JARVIS_GATEWAY_TOKEN="your-token"
jarvis onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env JARVIS_GATEWAY_TOKEN \
  --accept-risk
```
