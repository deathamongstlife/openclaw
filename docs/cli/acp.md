...
`jarvis acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
```bash
# One-shot request into your default Jarvis ACP session
acpx jarvis exec "Summarize the active Jarvis session state."
...
For a repo-local Jarvis checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an Jarvis agent without scraping a terminal.
