...
- `bindings[].agentId` is the owning Jarvis agent id.
...
- Jarvis ensures the configured ACP session exists before use.
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Jarvis session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Jarvis sandbox.
...
- Relative paths resolve from Jarvis workspace directory.
...
- Jarvis startup remains non-blocking while the backend health check runs.
...
```bash
jarvis config set plugins.entries.acpx.config.permissionMode approve-all
jarvis config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Jarvis currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
