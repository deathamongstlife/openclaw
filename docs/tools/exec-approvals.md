...
- For shell scripts and direct interpreter/runtime file invocations, Jarvis also tries to bind
...
These clients are opt-in. If a channel does not have exec approvals enabled, Jarvis does not treat
...
topics, Jarvis preserves the topic for the approval prompt and the post-approval follow-up.
