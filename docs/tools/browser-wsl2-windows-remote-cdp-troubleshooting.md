...
read_when:
  - Running Jarvis Gateway in WSL2 while Chrome lives on Windows
...
- Jarvis Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/jarvis/jarvis/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the Jarvis Chrome extension.
...
- Jarvis points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Jarvis is not the problem yet.
...
Fix that before touching Jarvis config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Jarvis at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Jarvis to succeed
...
```bash
jarvis browser open https://example.com --browser-profile remote
jarvis browser tabs --browser-profile remote
```

For the extension relay:

```bash
jarvis browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `jarvis browser tabs` returns the target
...
3. Jarvis config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Jarvis config or Control UI auth
