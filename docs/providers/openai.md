...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Jarvis.
...
OpenAI API usage. Jarvis forwards both through the `openai/*` Responses path.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Jarvis
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

Jarvis uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Jarvis also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Jarvis enables it by default for
...
Jarvis, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), Jarvis now auto-enables OpenAI server-side
