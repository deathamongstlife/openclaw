---
summary: "Use OpenCode Zen and Go catalogs with Jarvis"
read_when:
...
OpenCode exposes two hosted catalogs in Jarvis:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Jarvis keeps the runtime provider ids
...
```bash
jarvis onboard --auth-choice opencode-go
jarvis onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
