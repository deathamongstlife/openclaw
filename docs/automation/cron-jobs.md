...
- For upgrades, `jarvis doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
- CLI equivalents: `jarvis cron add --light-context ...` and `jarvis cron edit --light-context`.
...
When a job fails, Jarvis classifies errors as **transient** (retryable) or **permanent** (disable immediately).
