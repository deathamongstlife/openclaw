...
**分享输出：** 优先使用 `jarvis status --all`（它会隐藏令牌）。如果你粘贴 `jarvis status`，考虑先设置 `OPENCLAW_SHOW_SECRETS=0`（令牌预览）。
...
- 修复：从你希望服务使用的相同 `--profile` / `OPENCLAW_STATE_DIR` 重新运行 `jarvis gateway install --force`。
...
| Gateway 网关服务日志（监管程序） | macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.openclaw/logs/...`；配置文件使用 `~/.openclaw-<profile>/logs/...`）<br />Linux：`journalctl --user -u jarvis-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "Jarvis Gateway (<profile>)" /V /FO LIST` |
