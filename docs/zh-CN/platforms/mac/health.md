...
- 你仍然可以使用 [Gateway 网关健康](/gateway/health) 中的 CLI 流程（`jarvis status`、`jarvis status --deep`、`jarvis health --json`），并在 `/tmp/openclaw/openclaw-*.log` 中跟踪 `web-heartbeat` / `web-reconnect`。
