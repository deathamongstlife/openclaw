...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Jarvis.
...
- Install via CLI: `jarvis plugins install @openclaw/zalouser`
...
- When a group message is skipped because mention is required, Jarvis stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Jarvis state. Example:
...
- Jarvis sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Jarvis sends delivered + seen acknowledgements (best-effort).
...
- The channel now runs fully in Jarvis without external CLI binaries.
