...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Jarvis can stream partial replies in real time:
    ...
    - DM: Jarvis keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: Jarvis keeps the same preview message and performs a final edit in place (no second message)
    ...
    If native draft transport is unavailable/rejected, Jarvis automatically falls back to `sendMessage` + `editMessageText`.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - Jarvis pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    Telegram polls use `jarvis message poll` and support forum topics:

```bash
jarvis message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
jarvis message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, Jarvis preserves the topic for both the approval prompt and the post-approval follow-up.
...
- Multi-account precedence:
  ...
  - If neither is set, Jarvis falls back to the first normalized account ID and `jarvis doctor` warns.
