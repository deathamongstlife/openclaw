...
- `JARVIS_DOCKER_APT_PACKAGES` — install extra apt packages during image build
- `JARVIS_EXTENSIONS` — pre-install extension dependencies (space-separated extension names, e.g. `diagnostics-otel matrix`)
...
- **Persistent host data:** `JARVIS_CONFIG_DIR` and `JARVIS_WORKSPACE_DIR` are bind-mounted into the container and retain state on the host.
...
- **Disk growth hotspots:** the main paths to watch are `media/`, `agents/<agentId>/sessions/sessions.json`, transcript JSONL files, `cron/runs/*.jsonl`, and rolling file logs under `/tmp/jarvis/` (or your configured `logging.file`).
