...
- Local shell commands receive `JARVIS_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `JARVIS_THEME=light` before launching `jarvis tui`.
- To force the original dark palette instead, set `JARVIS_THEME=dark`.
