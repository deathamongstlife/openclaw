...
export type ConfigureCandidate = {
  ...
  pathSegments: string[];
  label: string;
  configFile: "jarvis.json" | "auth-profiles.json";
...
export function buildConfigureCandidates(config: JarvisConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: JarvisConfig;
  authoredJarvisConfig?: JarvisConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredJarvisConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const jarvisCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "jarvis.json" as const,
  ...
  return [...jarvisCandidates, ...authCandidates].toSorted((a, b) =>
