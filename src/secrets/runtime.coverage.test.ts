...
function buildConfigForJarvisTarget(entry: SecretRegistryEntry, envId: string): JarvisConfig {
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every jarvis.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "jarvis.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `JARVIS_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForJarvisTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `JARVIS_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
