    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "jarvis.plugin.json"),
      },
    ],
  };
}
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("jarvis-");
  const pluginRoot = await tempDirs.make("jarvis-plugin-");
  const outsideDir = await tempDirs.make("jarvis-outside-");
