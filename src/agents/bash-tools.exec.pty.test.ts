...
test("exec sets JARVIS_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-jarvis-shell", {
    command: "node -e \"process.stdout.write(process.env.JARVIS_SHELL || '')\"",
    pty: true,
  });
