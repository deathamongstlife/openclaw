...
describe("resolveApiKeyForProfile openai-codex refresh fallback", () => {
  const envSnapshot = captureEnv(["JARVIS_STATE_DIR", "JARVIS_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tempRoot = "";
  let agentDir = "";
  ...
  beforeEach(async () => {
    ...
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-codex-refresh-fallback-"));
    agentDir = path.join(tempRoot, "agents", "main", "agent");
    await fs.mkdir(agentDir, { recursive: true });
    process.env.JARVIS_STATE_DIR = tempRoot;
