...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-pdf-"));
  try {
...
function withDefaultModel(primary: string): JarvisConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as JarvisConfig;
}

function withPdfModel(primary: string): JarvisConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureJarvisModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ANTHROPIC_API_KEY", "anthropic-test");
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-pdf-out-"));
      try {
        const cfg = withDefaultModel(ANTHROPIC_PDF_MODEL);
