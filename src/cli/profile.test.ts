...
describe("parseCliProfileArgs", () => {
  it("leaves gateway --dev for subcommands", () => {
    const res = parseCliProfileArgs(["node", "jarvis", "gateway", "--dev", "--allow-unconfigured"]);
    if (!res.ok) {
...
describe("applyCliProfileEnv", () => {
  ...
  it("uses JARVIS_HOME when deriving profile state dir", () => {
    ...
    expect(env.JARVIS_CONFIG_PATH).toBe(path.join(resolvedHome, ".jarvis-work", "jarvis.json"));
  });
});
...
describe("formatCliCommand", () => {
  ...
  it("handles command with no args after jarvis", () => {
    expect(formatCliCommand("jarvis", { JARVIS_PROFILE: "test" })).toBe("jarvis --profile test");
  });
