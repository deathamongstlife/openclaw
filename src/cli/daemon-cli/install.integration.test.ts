...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("jarvis-daemon-install-int-");
    configPath = path.join(tempHome, "jarvis.json");
    process.env.HOME = tempHome;
    process.env.JARVIS_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.JARVIS_GATEWAY_TOKEN).toBeUndefined();
  });
});
