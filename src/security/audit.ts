...
import type { ConfigFileSnapshot, JarvisConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: JarvisConfig): boolean {
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.JARVIS_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.JARVIS_GATEWAY_PASSWORD) ||
...
function collectBrowserControlFindings(
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.JARVIS_GATEWAY_TOKEN) ||
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set JARVIS_GATEWAY_TOKEN/JARVIS_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("jarvis security audit --deep")}".`,
    });
  }
