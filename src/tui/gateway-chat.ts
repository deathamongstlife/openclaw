...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set JARVIS_GATEWAY_TOKEN/JARVIS_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.JARVIS_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.JARVIS_GATEWAY_PASSWORD);
