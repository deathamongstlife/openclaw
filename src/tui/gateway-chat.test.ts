...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `jarvis-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  it.each([
    ...
    const result = await withEnvAsync({ JARVIS_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("falls back to JARVIS_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers JARVIS_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "JARVIS_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-tui-file-secret-"));
