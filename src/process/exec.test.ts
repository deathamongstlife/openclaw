...
import { JARVIS_CLI_ENV_VALUE } from "../infra/jarvis-exec-env.js";
...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        JARVIS_BASE_ENV: "base",
        JARVIS_TO_REMOVE: undefined,
      },
      env: {
        JARVIS_TEST_ENV: "ok",
      },
    ...
    expect(resolved.JARVIS_BASE_ENV).toBe("base");
    expect(resolved.JARVIS_TEST_ENV).toBe("ok");
    expect(resolved.JARVIS_TO_REMOVE).toBeUndefined();
    expect(resolved.JARVIS_CLI).toBe(JARVIS_CLI_ENV_VALUE);
  });
