...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "jarvis.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  ...
  it("rejects packages without jarvis.extensions", async () => {
    ...
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_JARVIS_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when jarvis.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@jarvis/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing jarvis.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_JARVIS_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without jarvis.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@jarvis/voice-call@0.0.2-beta.1",
      name: "@jarvis/voice-call",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@jarvis/voice-call@beta"');
    }
  });
  ...
  it("allows explicit prerelease npm tags", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@jarvis/voice-call@0.0.2-beta.1");
    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@jarvis/voice-call@beta",
    });
    expect(packTmpDir).not.toBe("");
  });
});
