...
describe("GatewayClient security checks", () => {
  const envSnapshot = captureEnv(["JARVIS_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("allows ws:// to private addresses only with JARVIS_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with JARVIS_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.JARVIS_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://jarvis-gateway.ai:18789",
      onConnectError,
    });
