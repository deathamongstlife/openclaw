...
describe("gateway talk.config", () => {
  it("returns redacted talk config for read scope", async () => {
    ...
    await withServer(async (ws) => {
      ...
      expect(res.payload?.config?.talk?.providers?.elevenlabs?.apiKey).toBe("__JARVIS_REDACTED__");
      ...
      expect(res.payload?.config?.talk?.resolved?.config?.apiKey).toBe("__JARVIS_REDACTED__");
