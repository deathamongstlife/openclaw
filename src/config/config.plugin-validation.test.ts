...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    JARVIS_PLUGIN_MANIFEST_CACHE_MS: process.env.JARVIS_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-config-plugin-validation-"));
    ...
    process.env.JARVIS_STATE_DIR = path.join(suiteHome, ".jarvis");
    process.env.JARVIS_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.JARVIS_STATE_DIR === undefined) {
      delete process.env.JARVIS_STATE_DIR;
    } else {
      process.env.JARVIS_STATE_DIR = envSnapshot.JARVIS_STATE_DIR;
    }
    if (envSnapshot.JARVIS_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.JARVIS_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.JARVIS_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.JARVIS_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
