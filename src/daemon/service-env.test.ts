...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.JARVIS_GATEWAY_TOKEN).toBeUndefined();
    ...
    expect(env.JARVIS_WINDOWS_TASK_NAME).toBe("Jarvis Gateway");
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.JARVIS_WINDOWS_TASK_NAME).toBe("Jarvis Gateway (work)");
    if (process.platform === "darwin") {
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through JARVIS_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", JARVIS_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.JARVIS_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to JARVIS_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.JARVIS_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers JARVIS_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        JARVIS_GATEWAY_TOKEN: "jarvis-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.JARVIS_GATEWAY_TOKEN).toBe("jarvis-token");
  });

  it("omits JARVIS_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
