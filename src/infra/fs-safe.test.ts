...
describe("fs-safe", () => {
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("jarvis-fs-safe-root-");
    const sourceDir = await tempDirs.make("jarvis-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("jarvis-fs-safe-root-");
    const outside = await tempDirs.make("jarvis-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  ...
  it("reads a file via ~/path after HOME override", async () => {
    const root = await tempDirs.make("jarvis-tilde-test-");
    const originalHome = process.env.HOME;
  ...
  it("rejects ~/path that resolves outside root", async () => {
    const root = await tempDirs.make("jarvis-tilde-outside-");
