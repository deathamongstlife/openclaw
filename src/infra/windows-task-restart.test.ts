...
const resolvePreferredJarvisTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredJarvisTmpDirMock.mockReset();
  resolvePreferredJarvisTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  it("writes a detached schtasks relaunch helper", () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "Jarvis Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "Jarvis Gateway (work)" >nul 2>&1');
  ...
  it("prefers JARVIS_WINDOWS_TASK_NAME overrides", () => {
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Jarvis Gateway (custom)" >nul 2>&1');
  });
