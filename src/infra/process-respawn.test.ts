...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("returns supervised on macOS when launchd label is set (no kickstart)", () => {
    expectLaunchdSupervisedWithoutKickstart({ launchJobLabel: "ai.jarvis.gateway" });
  });
  ...
  it("returns supervised when XPC_SERVICE_NAME is set by launchd", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.jarvis.gateway";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(triggerJarvisRestartMock).not.toHaveBeenCalled();
    expect(spawnMock).not.toHaveBeenCalled();
  ...
  it("returns supervised when Jarvis gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.JARVIS_SERVICE_MARKER = "jarvis";
    process.env.JARVIS_SERVICE_KIND = "gateway";
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.JARVIS_TASK_SCRIPT = "C:\\jarvis\\node.cmd";
    process.env.JARVIS_TASK_SCRIPT_NAME = "node.cmd";
    process.env.JARVIS_SERVICE_MARKER = "jarvis";
    process.env.JARVIS_SERVICE_KIND = "node";
