...
import { JARVIS_CLI_ENV_VALUE } from "./jarvis-exec-env.js";
...
describe("sanitizeHostExecEnv", () => {
  it("removes dangerous inherited keys while preserving PATH", () => {
    ...
    expect(env).toEqual({
      JARVIS_CLI: JARVIS_CLI_ENV_VALUE,
      PATH: "/usr/bin:/bin",
      OK: "1",
    });
  });
  ...
  it("blocks PATH and dangerous override values", () => {
    ...
    expect(env.JARVIS_CLI).toBe(JARVIS_CLI_ENV_VALUE);
...
describe("git env exploit regression", () => {
  it("blocks GIT_SSH_COMMAND override so git cannot execute helper payloads", async () => {
    ...
    const marker = path.join(os.tmpdir(), `jarvis-git-ssh-command-${process.pid}-${Date.now()}`);
    try {
      fs.unlinkSync(marker);
    } catch {
      // no-op
