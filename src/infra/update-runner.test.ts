...
describe("runGatewayUpdate", () => {
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "jarvis.mjs")} doctor --non-interactive --fix`;
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g jarvis@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g jarvis@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "jarvis.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
