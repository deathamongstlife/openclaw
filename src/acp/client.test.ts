...
const createTempDir = () => tempDirs.make("jarvis-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets JARVIS_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "jarvis",
    });

    expect(env.JARVIS_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("jarvis");
  });

  it("overrides pre-existing JARVIS_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      JARVIS_SHELL: "wrong",
    });
    expect(env.JARVIS_SHELL).toBe("acp-client");
  ...
  it("preserves JARVIS_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        JARVIS_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["JARVIS_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Jarvis bridge", () => {
    ...
    expect(env.JARVIS_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.JARVIS_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "jarvis",
        serverArgs: ["acp"],
        defaultServerCommand: "jarvis",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("JARVIS_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "jarvis", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "jarvis", "dist", "entry.js");
    const shimPath = path.join(dir, "jarvis.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\jarvis\\dist\\entry.js" %*\r\n`, "utf8");
