...
type AgentLookupCache = {
  agentsRef: JarvisConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<JarvisConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: JarvisConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: JarvisConfig, agentId: string): string {
...
const resolvedRouteCacheByCfg = new WeakMap<
  ...
  {
    bindingsRef: JarvisConfig["bindings"];
    agentsRef: JarvisConfig["agents"];
    sessionRef: JarvisConfig["session"];
...
function resolveRouteCacheForConfig(cfg: JarvisConfig): Map<string, ResolvedAgentRoute> {
