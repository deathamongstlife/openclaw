...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["JARVIS_GATEWAY_TOKEN", await getenv("JARVIS_GATEWAY_TOKEN")]] as const;
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.JARVIS_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- JARVIS_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
