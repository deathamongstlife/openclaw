...
describe("backup commands", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("jarvis-backup-test-");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-workspace-link-"));
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    ...
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-backups-"));
    try {
      ...
      const extractDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-backup-extract-"));
      try {
  ...
  it("optionally verifies the archive after writing it", async () => {
    const stateDir = path.join(tempHome.home, ".jarvis");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-backup-verify-on-create-"));
    try {
      await fs.writeFile(path.join(stateDir, "jarvis.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".jarvis");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-backup-link-"));
  ...
  it("falls back to the home directory when cwd is a symlink into a backed-up source tree", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".jarvis");
    const workspaceDir = path.join(stateDir, "workspace");
    const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-backup-cwd-link-"));
