  ...
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("jarvis config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("jarvis config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
