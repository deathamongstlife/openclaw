...
describe("gateway-status command", () => {
  ...
  it("does not resolve local token SecretRef when JARVIS_GATEWAY_TOKEN is set", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      {
      ...
      configSnapshot: {
        ...
        config: {
          ...
          gateway: {
            ...
            auth: {
              mode: "token",
              token: { source: "env", provider: "default", id: "JARVIS_GATEWAY_TOKEN" },
              password: { source: "env", provider: "default", id: "JARVIS_GATEWAY_PASSWORD" },
            },
            remote: {
