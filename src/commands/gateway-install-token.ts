...
import { readConfigFileSnapshot, writeConfigFile, type JarvisConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("jarvis config set gateway.auth.mode token")} or ${formatCliCommand("jarvis config set gateway.auth.mode password")}.`,
  ].join(" ");
}
