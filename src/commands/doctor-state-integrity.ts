...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for JARVIS_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // JARVIS_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.jarvis).",
        `  Set locally: JARVIS_STATE_DIR=~/.jarvis ${formatCliCommand("jarvis doctor")}`,
      ].join("\n"),
    );
  }
