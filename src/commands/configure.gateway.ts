    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "JARVIS_GATEWAY_TOKEN",
          placeholder: "JARVIS_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like JARVIS_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Jarvis will store a token SecretRef.`, "Gateway token");
    } else {
