...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Jarvis", ".jarvis");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/jarvis-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".jarvis");
  ...
  it("anchors cloud detection to OS homedir when JARVIS_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".jarvis");
    const originalJarvisHome = process.env.JARVIS_HOME;
    process.env.JARVIS_HOME = "/tmp/jarvis-home-override";
