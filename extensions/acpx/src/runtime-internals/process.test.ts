...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "jarvis-acpx-process-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("spawnAndCollect", () => {
  ...
  it("strips shared provider auth env vars from spawned acpx children", async () => {
    ...
    vi.stubEnv("JARVIS_API_KEY", "keep-me");

    const result = await spawnAndCollect({
      command: process.execPath,
      args: [
        "-e",
        "process.stdout.write(JSON.stringify({openai:process.env.OPENAI_API_KEY,github:process.env.GITHUB_TOKEN,hf:process.env.HF_TOKEN,jarvis:process.env.JARVIS_API_KEY,shell:process.env.JARVIS_SHELL}))",
      ],
      cwd: process.cwd(),
    ...
    expect(parsed.jarvis).toBe("keep-me");
  ...
  it("strips provider auth env vars case-insensitively", async () => {
    ...
    const result = await spawnAndCollect({
      ...
      args: [
        "-e",
        "process.stdout.write(JSON.stringify({openai:process.env.OpenAI_Api_Key,github:process.env.Github_Token,jarvis:process.env.JARVIS_API_KEY,shell:process.env.JARVIS_SHELL}))",
      ],
      cwd: process.cwd(),
      stripProviderAuthEnvVars: true,
    });
