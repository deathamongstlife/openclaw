...
Read-only diff viewer plugin for **Jarvis** agents.
...
Set plugin-wide defaults in `~/.jarvis/jarvis.json`:
...
Before:
Jarvis supports plugins.

After:
Jarvis supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/jarvis-diffs`).
