...
# Jarvis Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.allyapp.cc/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Jarvis."
...
GUM_VERSION="${JARVIS_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Jarvis Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://jarvis.allyapp.cc/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$JARVIS_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.jarvis.allyapp.cc/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_jarvis_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.jarvis-* "$npm_root"/jarvis 2>/dev/null || true
}

extract_jarvis_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_jarvis_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/jarvis" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/jarvis"|"/usr/local/bin/jarvis")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/jarvis/"* ]]; then
            rm -f "$bin_path"
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Jarvis package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_jarvis_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Jarvis npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*jarvis" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_jarvis_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_jarvis_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Jarvis npm package installed"
                    return 0
                fi
...
map_legacy_env "JARVIS_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "JARVIS_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "JARVIS_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "JARVIS_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "JARVIS_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "JARVIS_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "JARVIS_BETA" "CLAWDBOT_BETA"
map_legacy_env "JARVIS_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "JARVIS_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "JARVIS_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "JARVIS_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "JARVIS_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "JARVIS_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${JARVIS_TAGLINE_INDEX:-}" ]]; then
        if [[ "${JARVIS_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((JARVIS_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${JARVIS_NO_ONBOARD:-0}
NO_PROMPT=${JARVIS_NO_PROMPT:-0}
DRY_RUN=${JARVIS_DRY_RUN:-0}
INSTALL_METHOD=${JARVIS_INSTALL_METHOD:-}
JARVIS_VERSION=${JARVIS_VERSION:-latest}
USE_BETA=${JARVIS_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/jarvis"
GIT_DIR=${JARVIS_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${JARVIS_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${JARVIS_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${JARVIS_VERBOSE:-0}"
JARVIS_BIN=""
SELECTED_NODE_BIN=""
PNPM_CMD=()
HELP=0
...
Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.allyapp.cc/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/jarvis)
...
Environment variables:
  ...
  JARVIS_VERSION=latest|next|<semver>
  JARVIS_BETA=0|1
  JARVIS_GIT_DIR=...
  ...
  JARVIS_NO_ONBOARD=1
  JARVIS_VERBOSE=1
  JARVIS_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.allyapp.cc/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.allyapp.cc/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.allyapp.cc/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Jarvis checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Jarvis source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_jarvis_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"jarvis"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://jarvis.allyapp.cc/install.sh | bash"
}
...
find_jarvis_entry_path() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" ]]; then
        return 1
    fi
    local entry_js="${npm_root}/jarvis/dist/entry.js"
    if [[ -f "$entry_js" ]]; then
        echo "$entry_js"
        return 0
    fi
    local entry_mjs="${npm_root}/jarvis/dist/entry.mjs"
    if [[ -f "$entry_mjs" ]]; then
        echo "$entry_mjs"
        return 0
    fi
...
install_jarvis_compat_shim() {
    ...
    local entry_path=""
    entry_path="$(find_jarvis_entry_path || true)"
    ...
    mkdir -p "$target_dir"
    local shim_path="${target_dir}/jarvis"
    cat > "$shim_path" <<EOF
#!/usr/bin/env bash
set -euo pipefail
...
EOF
    chmod +x "$shim_path"
    refresh_shell_command_cache
    ui_warn "Configured jarvis shim at ${shim_path} for Node $("$node_bin" -v 2>/dev/null || echo '22+')"
    return 0
}
...
detect_nvm_and_warn() {
    ...
    if [[ -n "$node_path" && "$node_path" == *".nvm"* ]]; then
        ...
        if [[ -n "$major" && "$major" -lt 22 ]]; then
            ...
            ui_warn "   Your shell is using NVM's Node ${current_version}, but Jarvis requires Node 22+"
            ui_warn ""
...
ensure_jarvis_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/jarvis" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/jarvis" ]]; then
        ln -sf "$npm_root/jarvis/dist/entry.js" "${npm_bin}/jarvis"
        ui_info "Created jarvis bin link at ${npm_bin}/jarvis"
    fi
    return 0
}

# Check for existing Jarvis installation
check_existing_jarvis() {
    if [[ -n "$(type -P jarvis 2>/dev/null || true)" ]]; then
        ui_info "Existing Jarvis installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_jarvis_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P jarvis 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/jarvis" ]]; then
        echo "${npm_bin}/jarvis"
        return 0
    fi
...
install_jarvis_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/jarvis/jarvis.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Jarvis from git checkout: ${repo_dir}"
    else
        ui_info "Installing Jarvis from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Jarvis" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Jarvis" run_pnpm -C "$repo_dir" build
...
EOF
    ...
    ui_success "Jarvis wrapper installed to \$HOME/.local/bin/jarvis"
...
install_jarvis() {
    local package_name="jarvis"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            JARVIS_VERSION="$beta_version"
    ...
    resolved_version="$(npm view "${package_name}@${JARVIS_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Jarvis v${resolved_version}"
    else
        ui_info "Installing Jarvis (${JARVIS_VERSION})"
    fi
    local install_spec=""
    if [[ "${JARVIS_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${JARVIS_VERSION}"
    fi

    if ! install_jarvis_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_jarvis_paths
        install_jarvis_npm "${install_spec}"
    fi

    if [[ "${JARVIS_VERSION}" == "latest" && "${package_name}" == "jarvis" ]]; then
        if ! resolve_jarvis_bin &> /dev/null; then
            ui_warn "npm install jarvis@latest failed; retrying jarvis@next"
            cleanup_npm_jarvis_paths
            install_jarvis_npm "jarvis@next"
        fi
    fi

    ensure_jarvis_bin_link || true

    ui_success "Jarvis installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${JARVIS_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_jarvis_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
...
resolve_workspace_dir() {
    ...
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.jarvis/workspace-${profile}"
    else
        echo "${HOME}/.jarvis/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${JARVIS_CONFIG_PATH:-$HOME/.jarvis/jarvis.json}"
...
resolve_jarvis_version() {
    local version=""
    local claw="${JARVIS_BIN:-}"
    if [[ -z "$claw" ]] && command -v jarvis &> /dev/null; then
        claw="$(command -v jarvis)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/jarvis/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/jarvis/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_jarvis_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Jarvis checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set JARVIS_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_jarvis_from_git "$repo_dir"
    else
        ...
        # Step 5: Jarvis
        install_jarvis
        install_jarvis_compat_shim || true
    fi

    ui_stage "Finalizing setup"

    JARVIS_BIN="$(resolve_jarvis_bin || true)"
    ...
    installed_version=$(resolve_jarvis_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Jarvis installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Jarvis installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/jarvis"
        ui_kv "Update command" "jarvis update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://jarvis.allyapp.cc/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                JARVIS_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                JARVIS_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                JARVIS_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
    ...
    if command -v jarvis &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ...
                if JARVIS_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
...
if [[ "${JARVIS_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
